<script lang="ts">
  import { findAccount, selected_corepage_id, accounts_config, hideSidebarMobile, isMobile } from '@/core/core.ts';
  ...
  let {
    showAddEditAccountModal = $bindable(false),
    showDelAccountModal = false,
    showExportModal = $bindable(false),
    showImportModal = $bindable(false),
    idItem = $bindable(null),
    accountTitle = $bindable(''),
  }: Props = $props();
        ...
        <TbodyTd title="Action">
          <TableActionItems>
            ...
            <Icon
              ...
              size="20px"
              padding="5px"
              onClick={() => clickDel(account.id, account.settings?.title)}
            />
          </TableActionItems>
        </TbodyTd>
      </TbodyTr>
...
<Modal
  title={idItem === null ? 'Add a new account' : 'Edit account'}
  body={ModalAccountsAddEdit}
  params={{ id: idItem || null }}
  bind:show={showAddEditAccountModal}
  width="fit-content"
/>
...
{#if showDelAccountModal}
  <Modal
    ...
    body={ModalAccountsDelete}
    params={{ id: idItem, name: accountTitle }}
    bind:show={showDelAccountModal}
  />
{/if}
