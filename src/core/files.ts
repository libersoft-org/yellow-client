...
import { rename, writeFile, open as openFile, create, exists, BaseDirectory } from '@tauri-apps/plugin-fs';
...
export async function offerNativeDownload(file_name: string, defaultFileDownloadFolder: string | null) {
 ...
 if (defaultFileDownloadFolder) {
  ...
  download.fd = await open(download.temp_file_path, { baseDir: BaseDirectory.Download, append: true });
  return download;
 }
}
...
export async function saveNativeDownloadChunk(download: NativeDownload, chunk: Blob) {
 ...
 await download.fd.write(new Uint8Array(await chunk.arrayBuffer()));
}
