...
{:else if type === 'inline'}
 {#if hideCopyButton}
  <span class:bx--snippet={true} class:bx--snippet--expand={expanded} class:bx--snippet--light={light} class:bx--snippet--no-copy={hideCopyButton} class:bx--snippet--wraptext={wrapText} class:bx--snippet--single={type === 'single'} class:bx--snippet--inline={type === 'inline'} class:bx--snippet--multi={type === 'multi'} {...$$restProps}>
   <code {id}>
    <slot>{code}</slot>
   </code>
  </span>
...
{:else}
 ...
 <div class:bx--snippet={true} class:bx--snippet--expand={expanded} class:bx--snippet--light={light} class:bx--snippet--no-copy={hideCopyButton} class:bx--snippet--wraptext={wrapText} class:bx--snippet--single={type === 'single'} class:bx--snippet--inline={type === 'inline'} class:bx--snippet--multi={type === 'multi'} class:bx--snippet--disabled={type !== 'inline' && disabled} {...$$restProps} on:mouseover on:mouseenter on:mouseleave>
  <!-- svelte-ignore a11y-no-noninteractive-tabindex -->
  <div role={type === 'single' ? 'textbox' : undefined} tabindex={type === 'single' && !disabled ? '0' : undefined} aria-label={$$restProps['aria-label'] || copyLabel || 'code-snippet'} class:bx--snippet-container={true} style:width="100%" style:min-height="{minHeight}px" style:max-height={maxHeight}>
   <pre bind:this={ref}><code><slot>{code}</slot></code></pre>
  </div>
  {#if !hideCopyButton}
   <CopyButton text={code} {copy} {disabled} {feedback} {feedbackTimeout} iconDescription={copyButtonDescription} on:click on:copy on:animationend />
  {/if}
  {#if showMoreLess}
   <Button
    kind="ghost"
