...
<div style:position="relative" style:z-index={open ? 1 : undefined} {...$$restProps}>
 {#if !hideIcon}
  <div bind:this={ref} id={triggerId} class:bx--tooltip__label={true}>
   <slot name="triggerText">{triggerText}</slot>
   <!-- svelte-ignore a11y-no-static-element-interactions -->
   <div bind:this={refIcon} {...buttonProps} aria-describedby={tooltipId} on:mousedown={onMousedown} on:focus={onFocus} on:keydown={onKeydown}>
    <slot name="icon">
 ...
 {:else}
  <!-- svelte-ignore a11y-no-static-element-interactions -->
  <div bind:this={ref} {...buttonProps} aria-describedby={tooltipId} on:mousedown={onMousedown} on:focus={onFocus} on:blur={onBlur} on:keydown={onKeydown}>
   <slot name="triggerText">{triggerText}</slot>
  </div>
 {/if}
 {#if open}
  <!-- svelte-ignore a11y-no-static-element-interactions -->
  <div bind:this={refTooltip} id={tooltipId} data-floating-menu-direction={direction} class:bx--tooltip={true} class:bx--tooltip--shown={open} class:bx--tooltip--top={direction === 'top'} class:bx--tooltip--right={direction === 'right'} class:bx--tooltip--bottom={direction === 'bottom'} class:bx--tooltip--left={direction === 'left'} class:bx--tooltip--align-center={align === 'center'} class:bx--tooltip--align-start={align === 'start'} class:bx--tooltip--align-end={align === 'end'} on:keydown={onKeydown}>
   <span class:bx--tooltip__caret={true}></span>
   <!-- svelte-ignore a11y-click-events-have-key-events -->
   <!-- svelte-ignore a11y-no-noninteractive-element-interactions -->
   <div on:click|stopPropagation on:mousedown|stopPropagation class:bx--tooltip__content={true} tabindex="-1" role="dialog">
    <slot />
   </div>
  </div>
 {/if}
