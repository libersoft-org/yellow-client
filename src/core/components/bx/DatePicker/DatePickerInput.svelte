...
<div class:bx--date-picker-container={true} class:bx--date-picker--nolabel={!labelText}>
 ...
 {/if}
 <div class:bx--date-picker-input__wrapper={true} class:bx--date-picker-input__wrapper--invalid={invalid} class:bx--date-picker-input__wrapper--warn={warn}>
  <input
   bind:this={ref}
   ...
   {disabled}
   {...$$restProps}
   value={$range ? ($inputIds.indexOf(id) === 0 ? $inputValueFrom : $inputValueTo) : $inputValue}
  ...
  />
  {#if invalid}
   <WarningFilled class="bx--date-picker__icon bx--date-picker__icon--invalid" />
  {/if}
  {#if !invalid && warn}
   <WarningAltFilled class="bx--date-picker__icon bx--date-picker__icon--warn" />
  {/if}
  {#if $hasCalendar && !invalid && !warn}
   <Calendar class="bx--date-picker__icon" aria-label={iconDescription} on:click={openCalendar} />
  {/if}
 </div>
 {#if invalid}
