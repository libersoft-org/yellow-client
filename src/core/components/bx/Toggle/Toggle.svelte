...
<div class:bx--form-item={true} style:user-select="none" {...$$restProps} on:click on:mouseover on:mouseenter on:mouseleave>
 ...
 <input
  ...
  {name}
 />
 <label aria-label={labelText ? undefined : $$props['aria-label'] || 'Toggle'} for={id} class:bx--toggle-input__label={true}>
  <span class:bx--visually-hidden={hideLabel}>
   <slot name="labelText">
    {labelText}
   </slot>
  </span>
  <span class:bx--toggle__switch={true} style:margin-top={hideLabel ? 0 : undefined}>
