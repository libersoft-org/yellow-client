...
<div class:bx--form-item={true} {...$$restProps} on:click on:mouseover on:mouseenter on:mouseleave>
 <label for={id} id={labelId} class:bx--label={true} class:bx--label--disabled={disabled} class:bx--visually-hidden={hideLabel}>
  <slot name="labelText">
   {labelText}
  </slot>
 </label>
 <div class:bx--slider-container={true} style:width={fullWidth && '100%'}>
  ...
  >
   <div role="slider" tabindex="0" class:bx--slider__thumb={true} style:left="{left}%" aria-valuemax={max} aria-valuemin={min} aria-valuenow={value} aria-labelledby={labelId} {id}></div>
   <div bind:this={trackRef} class:bx--slider__track={true}></div>
   <div class:bx--slider__filled-track={true} style:transform="translate(0, -50%) scaleX({left / 100})"></div>
  </div>
