<script>
 ...
 function sort() {
  if (selectionFeedback === 'top' || selectionFeedback === 'top-after-reopen') {
   const checkedItems = items.filter(item => selectedIds.includes(item.id)).map(item => ({ ...item, checked: true }));
   const uncheckedItems = items.filter(item => !selectedIds.includes(item.id)).map(item => ({ ...item, checked: false }));

   return [...(checkedItems.length > 1 ? checkedItems.sort(sortItem) : checkedItems), ...uncheckedItems.sort(sortItem)];
  }
 ...
 $: if (selectedIds && (selectionFeedback === 'top' || (selectionFeedback === 'top-after-reopen' && open === false))) {
  sortedItems = sort();
 }
 ...
 $: highlightedId = highlightedIndex > -1 ? ((filterable ? filteredItems : sortedItems)[highlightedIndex]?.id ?? null) : null;
</script>
...
<div bind:this={multiSelectRef} class:bx--multi-select__wrapper={true} class:bx--list-box__wrapper={true} class:bx--multi-select__wrapper--inline={inline} class:bx--list-box__wrapper--inline={inline} class:bx--multi-select__wrapper--inline--invalid={inline && invalid}>
