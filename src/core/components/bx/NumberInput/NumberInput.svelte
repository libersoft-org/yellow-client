...
<div class:bx--form-item={true} on:click on:mouseover on:mouseenter on:mouseleave>
 <div data-invalid={error || undefined} class:bx--number={true} class:bx--number--helpertext={true} class:bx--number--readonly={readonly} class:bx--number--light={light} class:bx--number--nolabel={hideLabel} class:bx--number--nosteppers={hideSteppers} class:bx--number--sm={size === 'sm'} class:bx--number--xl={size === 'xl'}>
  ...
  <div class:bx--number__input-wrapper={true} class:bx--number__input-wrapper--warning={!invalid && warn}>
   <input bind:this={ref} type="number" pattern="[0-9]*" aria-describedby={errorId} data-invalid={error || undefined} aria-invalid={error || undefined} aria-label={label ? undefined : ariaLabel} {disabled} {id} {name} {max} {min} {step} value={value ?? ''} {readonly} {...$$restProps} on:change={onChange} on:input={onInput} on:keydown on:keyup on:focus on:blur on:paste />
   {#if readonly}
   ...
   {:else}
    ...
    {/if}
    {#if !invalid && warn}
     <WarningAltFilled class="bx--number__invalid bx--number__invalid--warning" />
    {/if}
   {/if}
   {#if !hideSteppers}
