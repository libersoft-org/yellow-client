...
<ul {...$$restProps} role="tree" bind:this={ref} class:bx--tree={true} class:bx--tree--default={size === 'default'} class:bx--tree--compact={size === 'compact'} aria-label={hideLabel ? labelText : undefined} aria-labelledby={!hideLabel ? labelId : undefined} aria-multiselectable={selectedIds.length > 1 || undefined} on:keydown on:keydown|stopPropagation={handleKeyDown}>
