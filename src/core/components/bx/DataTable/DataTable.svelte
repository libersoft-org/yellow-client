<script>
 ...
 const formatHeaderWidth = header => {
  const styles = [header.width && `width: ${header.width}`, header.minWidth && `min-width: ${header.minWidth}`].filter(Boolean);
...
<TableContainer {useStaticWidth} {...$$restProps}>
 ...
 {/if}
 <slot />
 <Table {zebra} {size} {stickyHeader} {sortable} {useStaticWidth} tableStyle={hasCustomHeaderWidth && 'table-layout: fixed'}>
  <TableHead>
   <TableRow>
    {#if expandable}
      ...
      <TableHeader
       ...
       on:click={() => {
        ...
        if (header.sort === false) {
         ...
         sortKey = sortDirection === 'none' ? null : thKeys[header.key];
  ...
  <TableBody>
   {#each sorting ? displayedSortedRows : displayedRows as row, i (row.id)}
    ...
    >
     {#if expandable}
      <TableCell class="bx--table-expand" headers="expand" data-previous-value={!nonExpandableRowIds.includes(row.id) && expandedRows[row.id] ? 'collapsed' : undefined}>
