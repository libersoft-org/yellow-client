...
{#if paragraph}
 ...
 <div {...$$restProps} on:click on:mouseover on:mouseenter on:mouseleave>
  {#each Array.from({ length: lines }).map((_, i) => {
   ...
   <p class:bx--skeleton__text={true} class:bx--skeleton__heading={heading} style:width></p>
  {/each}
 </div>
{:else}
 <!-- svelte-ignore a11y-no-noninteractive-element-interactions -->
 <p class:bx--skeleton__text={true} class:bx--skeleton__heading={heading} style:width {...$$restProps} on:click on:mouseover on:mouseenter on:mouseleave></p>
{/if}
