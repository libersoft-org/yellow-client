...
{:else}
 ...
 <div class:bx--form-item={true} class:bx--checkbox-wrapper={true} {...$$restProps} on:click on:mouseover on:mouseenter on:mouseleave>
  <input
   bind:this={ref}
   type="checkbox"
   {value}
   ...
   on:change={() => {
    if (useGroup) {
     group = group.includes(value) ? group.filter(_value => _value !== value) : [...group, value];
    } else {
     checked = !checked;
    }
   }}
  ...
  <label for={id} {title} class:bx--checkbox-label={true}>
   <span bind:this={refLabel} class:bx--checkbox-label-text={true} class:bx--visually-hidden={hideLabel}>
    <slot name="labelText">
     {labelText}
    </slot>
   </span>
