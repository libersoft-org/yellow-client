...
<div bind:this={refSearch} class:bx--header__search={true} role="search" class:bx--header__search--active={active}>
 <label class:bx--header__search-label={true} for="search-input" id="search-label">Search</label>
 <div class:bx--header__search-menu={true} aria-owns="search-menu" aria-haspopup="menu">
  <button
   type="button"
   aria-label="Search"
 ...
 {#if active && results.length > 0}
  ...
  <ul aria-labelledby="search-label" role="menu" id="search-menu" class:bx--header-search-menu={true}>
   {#each results as result, i}
    <li role="none">
     <a
      ...
      class:bx--header-search-menu-item--selected={selectedId === `search-menuitem-${i}`}
     ...
     >
      <slot {result} index={i}>
       {result.text}
       {#if result.description}<span class:bx--header-search-menu-description={true}>– {result.description}</span>{/if}
      </slot>
     </a>
    </li>
   {/each}
