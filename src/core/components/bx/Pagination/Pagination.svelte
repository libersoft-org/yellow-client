<script>
  ...
  */
 export let itemRangeText = (min, max, total) => `${min}–${max} of ${total} item${max === 1 ? '' : 's'}`;
...
<div {id} class:bx--pagination={true} {...$$restProps}>
 <div class:bx--pagination__left={true}>
  {#if !pageSizeInputDisabled}
   <label id="bx--pagination-select-{id}-count-label" for="bx--pagination-select-{id}" class:bx--pagination__text={true}>
    {itemsPerPageText}
   </label>
   <Select
