...
<div on:click on:mouseover on:mouseenter on:mouseleave class:bx--form-item={true}>
 ...
 <div class:bx--text-area__wrapper={true} data-invalid={invalid || undefined}>
  ...
  {/if}
  <textarea bind:this={ref} bind:value aria-invalid={invalid || undefined} aria-describedby={invalid ? errorId : undefined} {disabled} {id} {name} {cols} {rows} {placeholder} {readonly} class:bx--text-area={true} class:bx--text-area--light={light} class:bx--text-area--invalid={invalid} maxlength={maxCount ?? undefined} {...$$restProps} on:change on:input on:keydown on:keyup on:focus on:blur on:paste></textarea>
 </div>
