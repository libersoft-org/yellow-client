...
<div class:bx--list-box__wrapper={true}>
 ...
 {/if}
 <ListBox class="bx--combo-box {direction === 'top' && 'bx--list-box--up'} {!invalid && warn && 'bx--combo-box--warning'}" id={comboId} aria-label={ariaLabel} {disabled} {invalid} {invalidText} {open} {light} {size} {warn} {warnText}>
  <ListBoxField
   role="button"
   aria-expanded={open}
   on:click={async () => {
  ...
  >
   ...
   {/if}
   {#if value}
    <ListBoxSelection on:clear on:clear={clear} translateWithId={translateWithIdSelection} {disabled} {open} />
   {/if}
   <ListBoxMenuIcon
    on:click={e => {
     if (disabled) return;
