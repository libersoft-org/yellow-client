...
{#if open}
 <div {role} {kind} class:bx--toast-notification={true} class:bx--toast-notification--low-contrast={lowContrast} class:bx--toast-notification--error={kind === 'error'} class:bx--toast-notification--info={kind === 'info'} class:bx--toast-notification--info-square={kind === 'info-square'} class:bx--toast-notification--success={kind === 'success'} class:bx--toast-notification--warning={kind === 'warning'} class:bx--toast-notification--warning-alt={kind === 'warning-alt'} style:width={fullWidth ? '100%' : undefined} {...$$restProps} on:click on:mouseover on:mouseenter on:mouseleave>
  ...
  {#if !hideCloseButton}
   <NotificationButton iconDescription={closeButtonDescription} on:click={close} />
  {/if}
 </div>
{/if}
