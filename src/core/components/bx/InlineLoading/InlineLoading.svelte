...
<div class:bx--inline-loading={true} aria-live="assertive" {...$$restProps} on:click on:mouseover on:mouseenter on:mouseleave>
 <div class:bx--inline-loading__animation={true}>
  {#if status === 'error'}
   <ErrorFilled class="bx--inline-loading--error" title={iconDescription || status} />
  {:else if status === 'finished'}
   <CheckmarkFilled class="bx--inline-loading__checkmark-container" title={iconDescription || status} />
