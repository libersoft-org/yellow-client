<script>
  ...
  let imagesRows = $derived.by(() => {
    ...
    children.forEach((child, index) => {
      ...
      if (
        groups.length === rowLimit &&
        groups[groups.length - 1].length === rowSize
      ) {
...
>
  {#each imagesRows as row, rowIndex (rowIndex)}
    <div
      class="images"
      style:margin-bottom={rowIndex === imagesRows.length - 1
        ? 0
        : "var(--images-gap)"}
    >
      {#each row as child, childIndex (child.tagUniqueId)}
        {@const isLastOfAll =
          rowIndex === rowLimit - 1 && childIndex === row.length - 1}
        <child.component
          {...child.props}
...
<style>
  ...
  .images :global(.message-content-image) {
    --image-size: min(
      120px,
      calc(
        60 * var(--messages-list-width) / 100 / 4 - var(--images-group-size) *
          var(--images-gap)
      )
    );
    display: flex;
