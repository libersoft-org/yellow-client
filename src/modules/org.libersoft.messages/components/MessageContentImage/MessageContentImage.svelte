<script>
  ...
  let yellowId = $derived(
    isYellow ? file.slice(YELLOW_SRC_PROTOCOL.length) : null,
  );
  ...
  fileUploadStore.store.subscribe(() =>
    upload.set(fileUploadStore.get(yellowId) || null),
  );
  ...
  function makeFilesForGallery() {
    ...
    for (let index = 0; index < siblings.length; index++) {
      ...
      const siblingYellowId =
        fileAttr && fileAttr.startsWith(YELLOW_SRC_PROTOCOL)
          ? fileAttr.slice(YELLOW_SRC_PROTOCOL.length)
          : null;
          ...
          loadFile: () =>
            new Promise((resolve, reject) => {
              filesService
                ...
                .then(({ localFile }) => {
                  if (
                    localFile.localFileStatus === LocalFileStatus.READY &&
                    localFile.fileBlob
                  ) {
                    const galleryFile = {
                      id: siblingYellowId,
  ...
  function downloadImage() {
    ...
    filesService
      ...
      .catch((err) => {
        console.error(
          "error fetching image data for yellow id:",
          yellowId,
          err,
        );
      });
  ...
  {#if isYellow}
    {#if $upload && $upload?.record.status !== FileUploadRecordStatus.FINISHED}
      <MessageContentAttachment
        node={{ attributes: { id: { value: yellowId } } }}
      />
    {:else}
      <div
