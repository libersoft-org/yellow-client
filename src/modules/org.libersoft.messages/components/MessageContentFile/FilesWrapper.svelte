<script>
  ...
  import {
    ...
    FileUploadRole,
  } from "@/org.libersoft.messages/services/fileUpload/types.ts";
  ...
  let downloadableRecords = $derived.by(() => {
    return attachedUploads.filter((upload) => {
      return (
        ...
        (upload.record.type === FileUploadRecordType.SERVER &&
  ...
  function getAttachmentEls() {
    return ref
      .closest(".attachments-wrap")
      .querySelectorAll(".message-attachment");
  }
...
<div class="attachments-wrap" bind:this={ref}>
  ...
  {#if showBulkDownloadAction}
    <div class="actions">
      <Button
        width="80px"
        text={uploadRecordType === FileUploadRecordType.P2P
          ? "Accept All"
          : "Download All"}
        onClick={onAcceptAll}
      />
