<script lang="ts">
  ...
  import {
    emoji_render,
    rgi_to_codepoints,
  } from "@/org.libersoft.messages/emojis";
  ...
  let tooltipButton = $state<{ ref: HTMLElement; reactions: any[] } | null>(
    null,
  );
  ...
  const renderInfoFromReactions = (reactions: any[]) => {
    ...
    const otherReactionAddresses = reactions
      .filter((r) => r.user_address !== myUserAddress)
      .map((r) => r.user_address);

    const emoji = emoji_render(
      rgi_to_codepoints(reactions[0].emoji_codepoints_rgi),
    );
    if (didIReact && otherReactionAddresses.length) {
      return (
        "You and " +
        otherReactionAddresses.join(", ") +
        " have reacted with " +
        emoji
      );
    } else if (didIReact) {
  ...
  $effect(() => {
    ...
    _union(added, removed, modified).forEach((a) =>
      highlightElement(buttonRefs[a]),
    );
    prevReactions = groupedReactions;
  });
</script>
