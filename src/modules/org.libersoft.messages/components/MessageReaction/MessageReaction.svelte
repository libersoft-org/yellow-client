<script lang="ts">
  ...
  import {
    ...
    identifier,
  } from "@/org.libersoft.messages/messages";
  ...
  const handleOutsideClick = (e: MouseEvent) => {
    if (
      floatingRef &&
      !floatingRef.contains(e.target as Node) &&
...
<div bind:this={buttonRef} class="reaction-button" class:open={show}>
  <BaseButton {onClick}>
    <img
      src="modules/{identifier}/img/add-reaction.svg"
      alt="Message settings"
      width="20px"
    />
  </BaseButton>
</div>
...
{#if show}
  <Portal>
    <div
      ...
      class="reaction-tooltip floating"
    ...
    >
      <div class="emojis">
        ...
        <BaseButton onClick={() => (showFull = !showFull)}>
          <div class="expand">
            <img
              src={showFull ? "img/close.svg" : "img/plus-white.svg"}
              alt="Message settings"
              width="18px"
            />
          </div>
