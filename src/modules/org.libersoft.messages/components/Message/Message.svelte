<script>
  import { deleteMessage, identifier, processMessage, setMessageSeen, toggleMessageReaction } from '../../messages.ts';
  import { debug } from '@/core/core.ts';
  import { onDestroy, onMount, tick } from 'svelte';
  import { isClientFocused } from '@/core/core.ts';
  import { stripHtml } from '../../messages.ts';
...
<div
  class="message {message.is_outgoing ? 'outgoing' : 'incoming'}"
  bind:this={elMessage}
  role="button"
  tabindex="0"
  ...
  on:contextmenu={rightClickContextMenu}
>
...
<ContextMenu bind:this={menu} target={elCaret}>
  ...
  <ContextMenuItem
    img="modules/{identifier}/img/reply.svg"
    label="Reply"
    onClick={replyMessage}
    data-testid="reply-context-menu-item"
  />
