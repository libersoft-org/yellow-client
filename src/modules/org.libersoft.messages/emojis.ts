...
interface EmojiAlternate {
  base: number[];
  alternates: number[][];
  codepoints_rgi?: string;
}

interface EmojiGroup {
  name: string;
  emoji: EmojiAlternate[];
}

interface EmojisByCodepointsRgi {
  [key: string]: EmojiAlternate;
}
...
export function start_emojisets_fetch(
  acc: AccountWithError,
  emojisLoading: Writable<boolean>,
  emojiGroups: Writable<EmojiGroup[]>,
  emojisByCodepointsRgi: Writable<EmojisByCodepointsRgi>
): void {
  ...
  new Promise<void>(async (resolve, reject) => {
    ...
    let by_codepoints: EmojisByCodepointsRgi = {};
...
export function replaceEmojisWithTags(text: string): string {
 return text.replace(emojiRegex, cluster => {
  ...
  let codepoints_array_text = rgi(cluster_array);
...
export async function init_emojis(): Promise<void> {
 ...
 for (let e of emojis) {
  for (let ee of e.emoji) {
   cp.push([
    // image url sequence
    ee.base.map(codepoint => codepoint.toString(16).padStart(4, '0')).join('_'),
    // emoji unicode
    ee.base.map(codepoint => String.fromCodePoint(codepoint)).join('')
   ]);
   for (let alt of ee.alternates) {
    cp.push([
     alt.map(codepoint => codepoint.toString(16).padStart(4, '0')).join('_'),
     alt.map(codepoint => String.fromCodePoint(codepoint)).join('')
    ]);
   }
  }
 }
 cpp = cpp.sort((a, b) => b[0].length - a[0].length);
 console.log(cpp);
