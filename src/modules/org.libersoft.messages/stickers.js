...
export async function updateStickerLibrary() {
 ...
 stickerLibraryUpdaterState.update(state => ({ ...state, status: `Update started ...`, updating: true, progress: 0, updated_once: true, error: null, canceled: false }));
 ...
 await stickers_db.transaction('rw', stickers_db.stickers, stickers_db.stickersets, async () => {
  ...
  stickerLibraryUpdaterState.update(state => ({ ...state, status: 'Removing old stickers from local database ...', progress: 10 }));
 ...
 for (let i = 0; i < sets.length; i++) {
  ...
  if (window.stickerLibraryUpdaterState.canceled) {
   console.log('Sticker library update canceled');
   stickerLibraryUpdaterState.update(state => ({ ...state, status: 'Sticker library update canceled', progress: 100, updating: false }));
   return;
  }
 ...
 stickerLibraryUpdaterState.update(state => ({ ...state, status: 'Sticker library updated', progress: 100, updating: false }));
}
