...
export async function updateStickerLibrary(stickerServer) {
 ...
 stickerLibraryUpdaterState.set({ status: 'Removing old stickers from local database ...', updating: true, progress: 3 });
 await db.transaction('rw', db.stickers, db.stickersets, async () => {
  ...
  stickerLibraryUpdaterState.set({ status: 'Removing old sticker sets from local database ...', updating: true, progress: 10 });
 ...
 for (let i = 0; i < sets.length; i++) {
  ...
  stickerset.url = stickerServer + '/api/sets?id=' + stickerset.id;
  //if (i % 100 === 0) {
  stickerLibraryUpdaterState.set({ status: 'Saving sticker set: ' + i + '/' + sets.length + ' to local database ...', updating: true, progress: (100 * i) / sets.length });
 ...
 stickerLibraryUpdaterState.set({ status: 'Loading stickers ...', updating: true, progress: 100 });
 await db.stickersets.bulkAdd(stickersets_batch);
